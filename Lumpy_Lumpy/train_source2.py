...
model_name = 'Lumpy-{}-cnn-{}-fc-{}-lr-{}-bz-{}-bn-{}-T_V_T-{}k_{}_{}-{}_{}_{}-itrs-{}-v2'\
...
with tf.Session() as sess:
	...
	for iteration in range(nb_steps):
		...
		if iteration%100 == 0:
			train_loss = src_clf_loss.eval(session=sess, feed_dict={xs:source_x, ys:source_y, is_training: False})
			val_loss = src_clf_loss.eval(session=sess, feed_dict={xs:Xs_val, ys:ys_val, is_training: False})
			test_loss = src_clf_loss.eval(session=sess, feed_dict={xs:Xs_tst, ys:ys_tst, is_training: False})
			train_stat = source_logit.eval(session=sess, feed_dict={xs:source_x, is_training: False}); train_auc = roc_auc_score(source_y, train_stat)
			val_stat = source_logit.eval(session=sess, feed_dict={xs:Xs_val, is_training: False}); val_auc = roc_auc_score(ys_val, val_stat)
			test_stat = source_logit.eval(session=sess, feed_dict={xs:Xs_tst, is_training: False}); test_auc = roc_auc_score(ys_tst, test_stat)
