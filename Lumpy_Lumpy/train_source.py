...
# load source data
Xs_trn, Xs_val, Xs_tst, ys_trn, ys_val, ys_tst = load_Lumpy(docker = docker, train = train, valid = valid, test = test, height = s_h, blur= s_blur, noise = s_noise)
...
with tf.Session() as sess:
	...
	for iteration in range(nb_steps):
		...
		if iteration%100 == 0:
			...
			train_stat = source_logit.eval(session=sess, feed_dict={xs:source_x}); src_trn_auc = roc_auc_score(source_y, train_stat)
			val_stat = source_logit.eval(session=sess, feed_dict={xs:Xs_tst}); src_auc = roc_auc_score(ys_tst, src_stat)
			test_stat = source_logit.eval(session=sess, feed_dict={xt:Xt_val}); trg_val_auc = roc_auc_score(yt_val, trg_val_stat)
			if nb_trg_labels > 0:
			...
			print(DA_model_name)
			if nb_trg_labels > 0:
				plot_AUC(DA_model_folder+'/auc-labels-{}.png'.format(DA_model_name), trg_trn_auc_list, trg_val_auc_list, trg_tst_auc_list)
				plot_LOSS(DA_model_folder+'/loss-labels-{}.png'.format(DA_model_name), trg_loss_list, src_loss_list, mmd_loss_list)
			plot_auc(DA_model_folder+'/auc-{}.png'.format(DA_model_name), trg_val_auc_list, trg_tst_auc_list)
			plot_loss(DA_model_folder+'/loss-{}.png'.format(DA_model_name), src_loss_list, mmd_loss_list)
			if best_val_auc < trg_val_auc:
				...
				plot_hist(DA_model_folder +'/hist-{}.png'.format(DA_model_name), trg_stat[:int(len(trg_stat)/2)], trg_stat[int(len(trg_stat)/2):])
