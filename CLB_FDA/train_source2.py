...
scope_name = 'base'
if clf_v == 1:
	conv_net, h, pred_logit = conv_classifier(x, nb_cnn = nb_cnn, fc_layers = [128,1],  bn = bn, scope_name = scope_name, bn_training = is_training)
else:
	conv_net, h, pred_logit = conv_classifier2(x, nb_cnn = nb_cnn, fc_layers = [128,1],  bn = bn, scope_name = scope_name, bn_training = is_training)

vars_list = tf.global_variables(scope_name)
key_list = [v.name[:-2] for v in tf.global_variables(scope_name)]
key_var_direct = {}
...
# Optimizer
if optimizer=="Adam":
  train_op = tf.train.AdamOptimizer(lr).minimize(cross_entropy, var_list = tf.trainable_variables('base'))
else:
  train_op= tf.train.GradientDescentOptimizer(lr).minimize(cross_entropy, var_list = tf.trainable_variables('base'))
...
with tf.Session() as sess:
	...
	for i_batch in range(num_steps):
		...
		sess.run(train_op, feed_dict={x: batch_x, y_: batch_y, is_training: True})
		if i_batch % 100 == 0:
			print('>>>>>>The {}-th batch >>>'.format(i_batch))
			train_logit = pred_logit.eval(session=sess,feed_dict={x:batch_x, is_training: False})
			train_loss = np.append(train_loss, cross_entropy.eval(session=sess, feed_dict={x:batch_x, y_:batch_y, is_training: False}))
			train_stat = np.exp(train_logit)
			train_auc = np.append(train_auc,roc_auc_score(batch_y, train_stat))

			test_logit = pred_logit.eval(session=sess,feed_dict={x:X_tst, is_training: False})
			test_loss = np.append(test_loss,cross_entropy.eval(session=sess, feed_dict={x:X_tst, y_:y_tst, is_training: False}))
			test_stat = np.exp(test_logit)
			test_auc = np.append(test_auc,roc_auc_score(y_tst, test_stat))

			val_logit = pred_logit.eval(session=sess,feed_dict={x:X_val, is_training: False})
			val_loss = np.append(val_loss,cross_entropy.eval(session=sess,feed_dict={x:X_tst, y_:y_val, is_training: False}))
