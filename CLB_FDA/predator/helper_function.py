...
# 	fig_size = (10,9)
	...
	ax.set_title('Discr last layer')
	ax.set_xlabel('Iteration ponts')
	ax.set_ylabel('Gradint mean square')
	ax.set_xlim([0,len(dis_grad_list1)])
	ax = fig.add_subplot(222)
	ax.plot(dis_grad_list2)
	ax.set_title('Discr first layer')
	ax.set_xlabel('Iteration ponts')
	ax.set_ylabel('Gradint mean square')
	ax.set_xlim([0,len(dis_grad_list2)])
	ax = fig.add_subplot(223)
	ax.plot(gen_grad_list1)
	ax.set_title('Gen last layer')
	ax.set_xlabel('Iteration ponts')
	ax.set_ylabel('Gradint mean square')
	ax.set_xlim([0,len(gen_grad_list1)])
	ax = fig.add_subplot(224)
	ax.plot(gen_grad_list2)
	ax.set_title('Gen first layer')
	ax.set_xlabel('Iteration ponts')
	ax.set_ylabel('Gradint mean square')
	ax.set_xlim([0,len(gen_grad_list2)])
	canvas = FigureCanvasAgg(fig)
	canvas.print_figure(file_name, dpi=100)
...
def plot_feature_pair_dist(file_name, source_feat, target_feat, source_y, target_y, label = ['source', 'target']):
	...
	sns.scatterplot(target_embedded[:,0], target_embedded[:,1], hue=target_y_list, legend='brief', palette=palette, marker = 'o')
	plt.savefig(file_name, dpi=100)
	plt.close('all')
