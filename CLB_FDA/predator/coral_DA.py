...
DA_model_name = 'coral-{0:}-lr-{1:}-bz-{2:}-iter-{3:}-scr-{4:}-shar-{5:}-fc-{6:}-bn-{7:}_v2'.format(mmd_param, lr, batch_size, nb_steps, source_scratch, shared, fc_layer, den_bn)
...
d = tf.cast(tf.shape(h_src)[1], tf.float32)

# Source covariance
xm = h_src - tf.reduce_mean(h_src, 0, keep_dims=True) 
xc = tf.matmul(tf.transpose(xm), xm) / batch_size

# Target covariance
xmt = h_trg - tf.reduce_mean(h_trg, 0, keep_dims=True) 
xct = tf.matmul(tf.transpose(xmt), xmt) / batch_size

coral_loss = tf.reduce_sum(tf.multiply((xc - xct), (xc - xct)))
coral_loss /= 4 * d * d

total_loss = coral_loss + src_clf_loss
...
with tf.Session() as sess:
	...
	for iteration in range(nb_steps):
		...
		if nb_trg_labels == 0:
			_, D_loss, C_loss = sess.run([gen_step, coral_loss, src_clf_loss], feed_dict={xs: batch_s, xt: batch_t, ys: batch_ys})
		else:
			batch_xt_l, batch_yt_l = Xt_trn_l[:nb_trn_labels, :], yt_trn_l[:nb_trn_labels, :]
			_, D_loss, C_loss = sess.run([gen_step, coral_loss, src_clf_loss], feed_dict={xs: batch_s, xt: batch_t, ys: batch_ys, xt1:batch_xt_l, yt1:batch_yt_l})
		#testing
		...
		print_yellow('Coral loss :{0:.4f}, Iter:{1:}'.format(D_loss, iteration))
		# save results
		...
		np.savetxt(os.path.join(DA_model_folder,'Coral_loss.txt'),D_loss_list)
