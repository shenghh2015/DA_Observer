...
parser.add_argument("--gp_param", type = int)
...
DA_model_name = 'wd-cnn-{0:}-fc-{1:}-bn-{2:}-bz-{3:}-D_lr-{4:}-G_lr-{5:}-nD-{6:}-nG-{7:}-iter-{8:}-gp-{9:}'.format(dis_cnn, dis_fc, dis_bn, batch_size, d_lr, g_lr, nd_steps, ng_steps, nb_steps, gp_param)
...
if dis_cnn > 0:
	...
	gradients = tf.gradients(src_logits, [conv_net_src])[0] + tf.gradients(trg_logits, [conv_net_trg])[0]
else:
	...
	gradients = tf.gradients(src_logits, [h_src])[0] + tf.gradients(trg_logits, [h_trg])[0]

wd_loss = (tf.reduce_mean(src_logits) - tf.reduce_mean(trg_logits))
slopes = tf.sqrt(tf.reduce_sum(tf.square(gradients), reduction_indices=[1]))
gradient_penalty = tf.reduce_mean((slopes-1.)**2)
disc_loss = -wd_loss+gp_param*gradient_penalty
...
gen_loss = -tf.reduce_mean(trg_logits)
...
with tf.Session() as sess:
	...
	for iteration in range(nb_steps):
		...
		for _ in range(nd_steps):
			_, D_loss = sess.run([disc_step, wd_loss], feed_dict={xs: batch_s, xt: batch_t})
...
# 		print_yellow('Loss: D {0:.4f}, G :{1:.4f}; Dom acc: {2:.4f} Iter:{3}'.format(D_loss, G_loss, domain_acc, iteration))
		print_yellow('Loss: D {0:.4f}, G :{1:.4f}, Iter:{2:}'.format(D_loss, G_loss, iteration))
		# save results
