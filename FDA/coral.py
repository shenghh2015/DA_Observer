...
DA_model_name = 'coral-dcnn-{}-gcnn-{}-fc-{}-dbn-{}-gbn-{}-clr-{}-glr-{}-dlr-{}-dw-{}-tw-{}-sw-{}-D-{}-L-{}-bz-{}-itr-{}'.\
...
#
# d = tf.cast(tf.shape(d_src_input)[1], tf.float32)
# 
# # Source covariance
# xm = d_src_input - tf.reduce_mean(d_src_input, 0, keep_dims=True) 
# xc = tf.matmul(tf.transpose(xm), xm) / batch_size
# 
# # Target covariance
# xmt = d_trg_input - tf.reduce_mean(d_trg_input, 0, keep_dims=True) 
# xct = tf.matmul(tf.transpose(xmt), xmt) / batch_size
# 
# coral_loss = tf.reduce_sum(tf.multiply((xc - xct), (xc - xct)))
# coral_loss /= 4 * d * d
h_s = d_src_input; h_t = d_trg_input
...
coral_loss = tf.nn.l2_loss(C_s - C_t)

# total loss
total_loss = s_weight*src_clf_loss + d_weight*coral_loss
# optimization ops
...
s_ops = tf.train.AdamOptimizer(lr).minimize(src_clf_loss, var_list=tf.trainable_variables('source'))
...
pseudo_label_train = args.pseudo_label
# D_loss_list = []
# G_loss_list = []
M_loss_list = []
sC_loss_list = []
...
with tf.Session() as sess:
	...
	for iteration in range(nb_steps):
		...
		_, sC_loss, M_loss, ttl_loss = sess.run([c_ops, src_clf_loss, coral_loss, total_loss], feed_dict={xs:batch_s,ys:batch_ys, xt:batch_t, g_training:True})
		if pseudo_label_train:
			trg_logits = target_logit.eval(session=sess,feed_dict={xt:batch_t, g_training:False})
			trg_probs = 1/(1 + np.exp(-trg_logits))
			psd_batch_one = batch_t[np.where(trg_probs.flatten()>0.7)[0],:]; psd_batch_zero= batch_t[np.where(trg_probs.flatten()<0.3)[0],:]
			min_len = min(psd_batch_one.shape[0], psd_batch_zero.shape[0])
			if min_len >0:
				psd_batch = np.concatenate([psd_batch_one[:min_len,:], psd_batch_zero[:min_len,:]], axis = 0); 
				psd_labels = np.concatenate([np.ones(min_len), np.zeros(min_len)]).reshape(-1,1);
				_ = sess.run([s_ops], feed_dict={xs:psd_batch,ys:psd_labels, g_training:True})
