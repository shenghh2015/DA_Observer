...
DA_model_name = 'wdgp-dcnn-{}-gcnn-{}-fc-{}-dbn-{}-gbn-{}-clr-{}-glr-{}-dlr-{}-dw-{}-tw-{}-sw-{}-D-{}-L-{}-bz-{}-itr-{}'.\
...
conv_net_src, h_src, source_logit = conv_classifier(xs, g_cnn,[128,1],g_bn,'source', bn_training = g_training)
conv_net_trg, h_trg, target_logit = conv_classifier(xt, g_cnn,[128,1],g_bn,'source', reuse = True, bn_training = g_training)
# discriminator input
if d_cnn > 0:
...
gen_loss = tf.reduce_mean(src_scores) - tf.reduce_mean(trg_scores)
dis_loss = tf.reduce_mean(trg_scores) - tf.reduce_mean(src_scores)
# 	mixed_loss = tf.reduce_sum(mixed_scores);
mixed_grads = tf.gradients(mixed_scores, [mixed_input])[0]
mixed_norms = tf.sqrt(tf.reduce_sum(tf.square(mixed_grads), axis=norm_reduce_shape))
gradient_penalty = tf.reduce_mean(tf.square(mixed_norms - wgan_target))
dis_loss += gradient_penalty * (wgan_lambda / (wgan_target**2))
epsilon_penalty = tf.reduce_mean(tf.square(src_scores))
dis_loss += epsilon_penalty * wgan_epsilon
...
c_ops = tf.train.AdamOptimizer(lr).minimize(src_clf_loss, var_list=tf.trainable_variables('source'))
...
def model_test(sess, source_logit, xs, Xs_tst):
	...
	while(batch_idx*minbatch_size<nb_sample):
		...
		logit_scores=source_logit.eval(session=sess,feed_dict={xs:batch,g_training:False,d_training:False})
...
with tf.Session() as sess:
	...
	for iteration in range(nb_steps):
		...
		_, sC_loss = sess.run([c_ops, src_clf_loss], feed_dict={xs:batch_s,ys:batch_ys,g_training:True,d_training: False})
		...
		if nb_trg_labels > 0:
			...
			_, tC_loss, trg_digit = sess.run([c_ops, src_clf_loss, target_logit], feed_dict={xs:batch_xt_l,ys:batch_yt_l,g_training:True,d_training: False})
...
# 			test_target_logit = target_logit.eval(session=sess,feed_dict={xt:Xt_tst, g_training: False, d_training: False})
			...
			val_target_logit = target_logit.eval(session=sess,feed_dict={xt:Xt_val, g_training: False, d_training: False})
			...
			# loss
			G_loss_list.append(G_loss);D_loss_list.append(D_loss);sC_loss_list.append(sC_loss)
			np.savetxt(DA_model_folder+'/D_loss.txt',D_loss_list);np.savetxt(DA_model_folder+'/G_loss.txt',G_loss_list);np.savetxt(DA_model_folder+'/src_clf_loss.txt',sC_loss_list)
			...
			if nb_trg_labels > 0:
				...
				print_yellow('Loss: D:{:.4f}, G:{:.4f}, S:{:.4f}, T:{:.4f}, Iter:{:}'.format(D_loss, G_loss,sC_loss, tC_loss, iteration))
				plot_LOSS(DA_model_folder+'/loss_{}.png'.format(DA_model_name), G_loss_list, sC_loss_list, tC_loss_list)
				plot_loss(DA_model_folder, D_loss_list, G_loss_list, DA_model_folder+'/DLOSS_{}.png'.format(DA_model_name))
				...
				print_yellow('Loss: D:{:.4f}, G:{:.4f}, S:{:.4f}, Iter:{:}'.format(D_loss, G_loss, sC_loss, iteration))
			...
			if iteration%1000 == 0:
				...
				source_feat = h_src.eval(session=sess,feed_dict={xs: Xs_tst[indices,],g_training:False,d_training:False}); target_feat = h_trg.eval(session=sess,feed_dict={xt:Xt_tst[indices,],g_training:False,d_training:False})
